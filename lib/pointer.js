...
Pointer.prototype.resolve = function(obj, options) {
  ...
  for (var i = 0; i < tokens.length; i++) {
    ...
    if (this.value[token] === undefined) {
      // fork version change this. When $ref pointer find undefined return string with example instead of throwing error.
      // throw ono.syntax('Error resolving $ref pointer "%s". \nToken "%s" does not exist.', this.path, token);
      this.value = {type: 'string',example:'Error resolving $ref pointer:'+token+' does not exist.'}
    }
    else {
